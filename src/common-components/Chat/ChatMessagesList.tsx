...
const Thinking = ({isThinking}: {isThinking?: boolean}) => useMemo(() => {
    if (isThinking) {
        return (
...
function ChatMessages({messages, isThinking}: ChatMessagesListProps) {
    ...
    return (<div>
        {messages.map((message, index) => message.isAI ? (
                    <div
                ...
                (
                    <ChatBubble end={message.isUser} key={`${message.text}_${index}`}>
                        <ChatBubble.Header>Obi-Wan name <ChatBubble.Time>12:45</ChatBubble.Time>
                        </ChatBubble.Header>
